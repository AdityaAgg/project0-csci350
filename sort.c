...
int compare_ab_reverse(const void *a, const void *b) {
    const uint32_t * aa = (const uint32_t *) a;
    const uint32_t *bb = (const uint32_t *) b;
    return *bb - *aa;
}

int compare_set_bits(const void *a, const void *b) {
    uint32_t a_value = *(const uint32_t *)a;
    uint32_t b_value = *(const uint32_t *)b;
    int a_amount = 0;
    int b_amount = 0;
    for(int i = 0; i<32; ++i) {
    ...
    //printf("for %d a_value and %d b_value a is %d \n \n", a_amount, b_amount, a_amount-b_amount);
